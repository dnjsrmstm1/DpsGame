...
type 보석타입 = '하급' | '중급' | '상급' | '특급' | '고급' | '재물' | '경험보석' | '보호보석' | '궁극' | '수호' | '초월보석' | '인내' | '강타' | '자동화' | '채광' | '채굴' | '섬세'
type 보석목록 = { [K in 보석타입]: number }
const 초기보석: 보석목록 = { 하급: 0, 중급: 0, 상급: 0, 특급: 0, 고급: 0, 재물: 0, 경험보석: 0, 보호보석: 0, 궁극: 0, 수호: 0, 초월보석: 0, 인내: 0, 강타: 0, 자동화: 0, 채광: 0, 채굴: 0, 섬세: 0 }

// 사용자 지정 초기비용 (무색조각). 동적: base × (1 + 보유/10)
const 보석구입비용: Record<보석타입, number> = {
  하급: 1, 중급: 1, 상급: 1, 특급: 1, 고급: 1,
  재물: 1, 경험보석: 2, 보호보석: 100, 궁극: 100,
  채광: 10000, 채굴: 5000000, 수호: 5000000, 초월보석: 4000000,
...
const 보석max: Record<보석타입, number> = {
  ...
  강타: 100, 자동화: 100, 채광: 100, 채굴: 100, 섬세: 100,
}
...
function 보석보너스합산(b: 보석목록) {
  ...
  // 채굴: 크리스탈조각 복리 (10%/개)
  const 조각배수보석 = Math.pow(1.1, b.채굴)
  return { add44, add45, add46, add47, add48, 파괴방지, 자원배수추가, 경험배수, 초월확률추가, 보스수보너스, 자동속도배수, 크레딧배수, 조각배수보석 }
}
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      for (const s of 판매수집) {
        ...
        판매크리조각 += Math.round(r.크리스탈조각 * 판매보상배수 * 조각배수 * 보석b.조각배수보석)
    ...
    >
          ...
          <ScrollView style={{ maxHeight: 360 }}>
            ...
            {스텟탭 === '보석' && (() => {
              const 보석정보in: { 종류: 보석타입; 이모지: string; 설명: string; 효과: string }[] = [
                ...
                { 종류: '채굴',    이모지: '⚒️', 설명: '채굴의 보석',       효과: '크리스탈조각 ×1.1/회 (복리)' },
      ...
      {명칭크리스탈패널열림 && (() => {
        ...
        const 크리스탈목록표: Record<크탭, 크정보[]> = {
          ...
          유니크: [
            { 키: '흑색', 이름: '흑색의 크리스탈', 비용: 500, 색상: '#7f8c8d', 효과: `하락방어 60h + 무색 ×${Math.pow(2, 명칭크리스탈.흑색).toFixed(2)} + 판매 ×${Math.pow(1.10, 명칭크리스탈.흑색).toFixed(2)} + 초월확률 +${(명칭크리스탈.흑색 * 0.5).toFixed(1)}%`, 구현: true },
